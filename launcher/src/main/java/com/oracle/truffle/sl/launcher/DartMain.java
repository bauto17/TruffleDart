...
public final class DartMain {
    ...
    public static void main(String[] args) throws IOException {
        ...
        if (file == null) {
            // @formatter:off
            source = Source.newBuilder(DART, new InputStreamReader(System.in), "<stdin>").build();
            // @formatter:on
        } else {
            source = Source.newBuilder(DART, new File(file)).build();
        }
    ...
    private static int executeSource(Source source, InputStream in, PrintStream out, Map<String, String> options) {
        ...
        try {
            context = Context.newBuilder(DART).in(in).out(out).options(options).build();
        ...
        try {
            ...
            out.println("== running on SL lalalalal");
            if (context.getBindings(DART).getMember("main") == null) {
