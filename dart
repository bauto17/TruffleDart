...
# Asegurar que es igual ala valor de graalvm.version en ./pom.xml
VERSION="22.0.0.2"

MAIN_CLASS="edu.uniandes.dart.launcher.DartMain"
...
if [[ "$GRAALVM_VERSION" != "" ]]; then
    ...
    LAUNCHER_PATH="$SCRIPT_HOME/../jre/languages/dart/launcher/dart-launcher.jar:$SCRIPT_HOME/../languages/dart/launcher/dart-launcher.jar"
    JAVACMD="$SCRIPT_HOME/java"
    versionCheck "Installed in wrong version of GraalVM. Expected: $VERSION, found $GRAALVM_VERSION"
else
    LANGUAGE_PATH="$SCRIPT_HOME/language/target/dartlanguage-$VERSION.jar"
